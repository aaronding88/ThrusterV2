...
public class GameOverScript : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		if (highScore > 0)
		{
			...
			// Sets the following textfield under this Control Name.
			...
			// Automatically transfers the focus to the Textfield. Keeps the user from having to use tab to select it.
			GUI.FocusControl("nameTextField");
			// Submit button.
			if (GUI.Button(new Rect(Screen.width / 2 - (buttonWidth / 2),
			                        2 *Screen.height / 3 - (buttonHeight / 2),
			                        buttonWidth,
			                        buttonHeight), "Submit!", gameOverStyle) || Event.current.keyCode == KeyCode.Return)
			{
